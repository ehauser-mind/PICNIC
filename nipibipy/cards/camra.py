...
from ..workflows.camra_workflows import (
    LcfCamraWorkflow,
)
...
class Camra(CardBuilder):
    ...
    def _check_parameter_syntax(self):
        ...
        assert self._type in AVAILABLE_TYPES, 'Error: Unsupported type '+self._type+' in '+self._name+' keyword'
        assert self._cost in AVAILABLE_COSTS, 'Error: Unsupported cost '+self._cost+' in '+self._name+' keyword'
        ...
        assert isinstance(self._ct, bool), 'Error: CAMRA parameter: ct must be a boolean (True or False)'
        self._rank = self._force_parameter_to_integer(self._rank, 'rank')
        assert isinstance(self._report, bool), 'Error: CAMRA parameter: report must be a boolean (True or False)'
