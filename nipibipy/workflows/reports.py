...
def create_report(report_type, files, basename='', extras=[]):
    ...
    """
    # imports
    import nibabel, sys, os, numpy, json, pandas, tempfile
    import matplotlib
    ...
    def image_summary(in_file, basename='image'):
        ...
        if len(image.shape)==4:
            ...
            mean_image = nibabel.Nifti1Image(numpy.mean(fdata, axis=3), image.affine)
            ...
            for frame in range(fdata.shape[3]):
                slice_image = nibabel.Nifti1Image(fdata[:,:,:,frame], image.affine)
    ...
    def motion_correction_summary(base_file, moco_file, mats=None, ref_frame=None, basename='motion_correction'):
        ...
        if mats is not None:
            ...
            for mat in mats:
                ...
                all_dof.append(numpy.array([x, y, z, rx, ry, rz]))
            all_dof = numpy.array(all_dof)
        
        # get ready to create some motion correction graphs
        base_fdata = base_image.get_fdata()
        moco_fdata = moco_image.get_fdata()
        clim = advanced_colorbar_limits(nibabel.Nifti1Image(numpy.mean(base_fdata, axis=3), base_image.affine))
        t = numpy.arange(base_fdata.shape[3])
        ...
        for frame in range(base_fdata.shape[3]):
            ...
            for img in (base_fdata, moco_fdata):
                coords.append(plot_motion_correction_image(nibabel.Nifti1Image(img[:,:,:,frame], base_image.affine), cut_coords_bounds, cmap='jet', vmin=clim[0], vmax=clim[1]))
                
                # draw lines on the brain images
                i = Image.open(coords[-1])
                w, h = i.size
                draw = ImageDraw.Draw(i)
                for line_height in numpy.linspace(0, h, 9)[1:-1]:
                    draw.line((0, line_height, w, line_height), width=1)
                for line_width in numpy.linspace(0, w, 23)[1:-1]:
    ...
    def coregistration_summary(base_file, overlay_file, basename='coregistration'):
        ...
        for fn in (base_file, overlay_file):
            img = nibabel.load(fn)
            if len(img.shape)==4:
                img = nibabel.Nifti1Image(numpy.mean(img.get_fdata(), axis=3), img.affine)
            imgs.append(img)
        ...
        assembled_images = []
        for alpha in numpy.linspace(0.0, 0.7, 6):
    ...
    def camra_summary(base_file, overlay_files, rank_list, mats=None, rank=1, basename='motion_correction'):
        ...
        if mats is not None:
            all_dof = []
            for idx in sorted_rank_list:
                m = numpy.loadtxt(mats[rank_list.index(idx)])
                x, y, z = m[0,3], m[1, 3], m[2, 3]
                rx = degrees(atan2(-m[1, 2], m[2, 2]))
                ry = degrees(asin(m[0, 2]))
                rz = degrees(atan2(-m[0, 1], m[0, 0]))
    ...
    def tacs_summary(tacs_file, units, basename='tacs'):
        ...
        """
        # load tacs files
        data = pandas.read_csv(tacs_file, delimiter='\t', header=0, index_col=0)
    ...
    def plot_anat(image, cut_coords_bounds, cmap='gray', vmin=None, vmax=None, n_cuts=7):
        ...
        for direction in 'xyz':
            ...
            _ = plot_anat(
                ...
                cut_coords=numpy.linspace(*cut_coords_bounds[direction], n_cuts), 
    ...
    def plot_image_overlay(overlay_image, base_image, cut_coords_bounds, direction='y', cmap='gray', n_cuts=5, alpha=0.7, vmin=None, vmax=None):
        ...
        """
        # create the temporary images for each direction (x, y and z)
        all_coords = numpy.linspace(*cut_coords_bounds[direction], 3*n_cuts+1)
        coords = []
        for line in range(3):
    ...
    def plot_motion_correction_image(image, cut_coords_bounds, cmap='jet', vmin=None, vmax=None):
        ...
        """
        # create the temporary images for each direction (x, y and z)
        ortho_cut_coords = [numpy.mean(cut_coords_bounds[direction]) for direction in 'xyz']
    ...
    def plot_motion_correction_graph(t, all_dof, frame, ref_frame=None):
        ...
        _ = ax.vlines(t[frame], numpy.min(all_dof[:, :3]), numpy.max(all_dof[:, :3]), colors='r', linestyles='dashed', label='current frame')
        if ref_frame is not None:
            _ = ax.vlines(ref_frame, numpy.min(all_dof[:, :3]), numpy.max(all_dof[:, :3]), colors='g', linestyles='dashed', label='reference frame')
    ...
    def plot_camra_graph(t, all_dof, idx, autoselect=None):
        ...
        # create vertical lines for the current and reference frame
        _ = ax.vlines(idx, numpy.min(all_dof[:, :3]), numpy.max(all_dof[:, :3]), colors='r', linestyles='dashed', label='Current')
        if autoselect is not None:
            _ = ax.vlines(autoselect, numpy.min(all_dof[:, :3]), numpy.max(all_dof[:, :3]), colors='g', linestyles='dashed', label='Autoselected')
    ...
    def advanced_colorbar_limits(image):
        ...
        weight_array = numpy.zeros(fdata.shape)
        x, y, z = weight_array.shape
        for i in range(x):
            for j in range(y):
                for k in range(z):
                    weight_array[i, j, k] = numpy.prod([sin((float(a/b) * pi / 2.) + (pi / 4.)) for a, b in zip([i, j, k], [x, y, z])])
        weighted_fdata = weight_array * fdata
        return (0., numpy.percentile(weighted_fdata[weighted_fdata > 0.], 99.5))
